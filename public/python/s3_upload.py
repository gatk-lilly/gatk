...
def upload(bucketname, source_path, keyname, acl='private', headers={}, guess_mimetype=True, parallel_processes=33):
    """
    ...
    log("Uploading %s, %s bytes in %s chunks" % (source_path, source_size, chunk_amount))
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
         ...
         script_name = "s3_upload.py"         
         bucket_name = sys.argv[1]
         dir_path = sys.argv[2]
         s3_path = sys.argv[3]
         ext = sys.argv[4]

         file_paths = get_file_paths(dir_path, "*" + ext)
        ...
        help = """
usage: python s3_upload.py [s3 bucket name] [directory path] [s3_path] [extension]
